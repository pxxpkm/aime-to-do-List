...
String? normalizePosterUrl(String? url) {
  ...
  } else if (s.startsWith('http://lain.bgm.tv') || s.startsWith('http://bgm.tv')) {
...
/// Local proxy base for Flutter Web poster loading (see proxy_server.py).
const String kLocalPosterProxyBase = 'http://127.0.0.1:8080/proxy?url=';

/// Rewrite Bangumi CDN URLs through the local CORS proxy (Web only call site).
String toProxyUrl(String url) {
  if (url.startsWith('data:')) return url;
  if (url.contains('/proxy?url=')) return url;
  if (url.contains('lain.bgm.tv') || url.contains('bgm.tv')) {
    return '$kLocalPosterProxyBase${Uri.encodeComponent(url)}';
  }
  return url;
}
